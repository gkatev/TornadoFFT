 ...
 * radix method and has special methods to handle factors [2, 3, 4, 5, 6, 7] and a general method
 * for larger prime factors.
 *
...
public class Complex {
	...
	/**
	 * Construct a Complex instance for data of length n. Factorization of n is designed to use
	 * special methods for small factors, and a general routine for large odd prime factors. Scratch
	 * memory is created of length 2*n, which is reused each time a tranform is computed.
	 *
		...
		for(@Parallel int i = 0; i < factors.length; i++) {
			...
			for(@Parallel int k = 1; k <= q[i]; k++) {
				...
				for(int j = 0; j < factors[i] - 1; j++) {
					...
					twid[i][k][2 * j] = cos(theta);
					twid[i][k][2 * j + 1] = sin(theta)
				}
			}
		}
	...
	private static void wavetable_sp(int n, int[] factors, double[][][] twid) {
		if (n < 2) {
			return;
		}
		...
		// twid = new double[factors.length][][];
		...
		for (int i = 0; i < factors.length; i++) {
			...
			twid[i] = new double[q + 1][2 * (factor - 1)];
			...
			// System.out.println("wavetable: n = " + n + ", product = " + product + ", q = " + q);
